...
def create_model(
    ...
    vggnet_layers = list(vggnet.features)
    vggnet_index = 0
        ...
        for model_type in model_type_order:
            if model_type == "Weights":
                ...
                if isinstance(model_, torch.nn.Conv2d):
                    while not isinstance(vggnet_layers[vggnet_index], torch.nn.Conv2d):
                        vggnet_index += 1
                    model_.weight.data = vggnet_layers[vggnet_index].weight.data.clone()
                    if model_.bias is not None:
                        model_.bias.data = vggnet_layers[vggnet_index].bias.data.clone()
                ...
                if cnn_key not in  ["cnn_2", "cnn_4", "cnn_6"]:

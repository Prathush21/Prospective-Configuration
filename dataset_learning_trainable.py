...
class DatasetLearningTrainable(BaseTrainable):
    ...
    def reset_data_packs(self):
        ...
        self.is_data_loader_in_raypg = self.config.get("is_data_loader_in_raypg", False)
        ...
        for data_pack_key, data_pack in self.data_packs.items():
            ...
            assert isinstance(
                data_pack["data_loader"], str
            ), f"data_loader should be a string, but got {data_pack['data_loader']}"
            # data_loader should be eval
            data_loader = eval(data_pack["data_loader"])

            data_pack["data_loader"] = (
                ray.put(data_loader) if self.is_data_loader_in_raypg else data_loader
            )

            # by default, it takes effect at all iterations
            at_iteration = data_pack.get("at_iteration", "'all'")
            assert isinstance(
                at_iteration, str
            ...
            if isinstance(data_pack["at_iteration"], list):
                ...
                assert data_pack["at_iteration"] in [
                    "all"
            ...
            assert isinstance(do, str), f"do should be a string, but got {do}"
    ...
    def reset_log_packs(self):
        ...
        for _, log_pack in self.log_packs.items():
            ...
            # # by default, it takes effect at all iterations
            at_iteration = log_pack.get("at_iteration", "'all'")
            assert isinstance(
                at_iteration, str
            ), f"at_iteration should be a string, but got {at_iteration}"
            ...
            if isinstance(log_pack["at_iteration"], list):
                ...
                assert log_pack["at_iteration"] in [
                    "all"
            ...
            at_batch_idx = log_pack.get("at_batch_idx", "'all'")
            assert isinstance(
                at_batch_idx, str
            ), f"at_batch_idx should be a string, but got {at_batch_idx}"
            log_pack["at_batch_idx"] = eval(at_batch_idx)
            if isinstance(log_pack["at_batch_idx"], str):
                assert log_pack["at_batch_idx"] in [
                    "all"
            ...
            assert isinstance(
                summarize_over_batch_idx_fn, str
            ), f"summarize_over_batch_idx_fn should be a string, but get {summarize_over_batch_idx_fn}"
            log_pack["summarize_over_batch_idx_fn"] = eval(summarize_over_batch_idx_fn)
            assert callable(
                log_pack["summarize_over_batch_idx_fn"]
    ...
    def reset_config(self, new_config):
        ...
        exec(
            self.config.get("before_DatasetLearningTrainable_reset_config_code", "pass")
        )
        ...
        exec(
            self.config.get("after_DatasetLearningTrainable_reset_config_code", "pass")
        )
    ...
    def step(self):
        ...
        for data_pack_key, data_pack in self.data_packs.items():
            ...
            if (data_pack["at_iteration"] == "all") or (
                self._iteration in data_pack["at_iteration"]
            ):

                if (
                    self.config.get("before_iteration_data_loader_code", None)
                    is not None
                ):
                ...
                for batch_idx, batch in enumerate(
                    ray.get(data_pack["data_loader"])
                    if self.is_data_loader_in_raypg
                    else data_pack["data_loader"]
                ):

                    # get batch and put it to device
                    batch = list(batch)
                    for batch_item_i in range(len(batch)):
                        batch[batch_item_i] = batch[batch_item_i].to(self.device)
                    batch = tuple(batch)
                    ...
                    for do_key in data_pack["do"]:
                        ...
                        if isinstance(step_iteration_return, dict):
                            step_iteration_result_dict.update(step_iteration_return)
                    ...
                    for log_pack_key, log_pack in self.log_packs.items():
                        ...
                        if (log_pack["at_iteration"] == "all") or (
                            self._iteration in log_pack["at_iteration"]
                        ):
                            # only take effect at specific data_pack
                            if data_pack_key in log_pack["at_data_pack"]:
                                # only take effect at specific batch_idx
                                if (
                                    isinstance(log_pack["at_batch_idx"], str)
                                    and log_pack["at_batch_idx"] == "all"
                                ) or (
                                    isinstance(log_pack["at_batch_idx"], list)
                                    and batch_idx in log_pack["at_batch_idx"]
                                ):
                                    if "log_fn" in log_pack.keys():
                ...
                if (
                    self.config.get("after_iteration_data_loader_code", None)
                    is not None
                ):
                ...
                for result_key in result_dict.keys():
                    result_dict[result_key] = self.log_packs[result_key.split("__")[1]][
        ...
        if self._iteration == 0:
            for log_key_holder in self._log_key_holders:
                if ":" in log_key_holder:
                    ...
                    log_key_holder = log_key_holder.replace(": ", "__").replace(
                        ":", "__"
                    )
